...
production SSB 用途に**足りないもの**は automatic frequency
control (AFC): 現状の rx は `audio_centre_hz` を厳密に既知の
前提だが、実 SSB 受信機は TX/RX VFO 差で中心が ±50–100 Hz
ずれる。AFC は ~50–100 行の追加（preamble 相関器を周波数で
sweep、その offset を LMS 位相フィットを通して追跡）で実装
可能 — 次の cycle で対応予定、0.3.1 ではない。

AFC が入るまで SSB 利用には両端で ~10 Hz 以内に dial を合わせる
必要あり。デジタル VFO + CAT 制御なら簡単、マニュアルダイヤル
なら厳しい。
