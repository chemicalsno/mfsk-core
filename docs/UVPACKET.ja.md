...
| | チャンネル | FEC | Net bps | 50 % PER 閾値 | OSS |
|---|---|---|---:|---:|:-:|
...
| **uvpacket Robust** | **NFM または SSB** | **LDPC + OSD** | **1008** | **−3.7 dB SNR_3kHz** | ✓ |
| uvpacket Express | NFM または SSB | LDPC + OSD | 1800 | ~ −1 dB SNR_3kHz | ✓ |
...
modem は audio 領域の QPSK + RRC プロセッサ（信号は 1500 Hz
中心の ~1200–1800 Hz、典型的な SSB パスバンドに余裕で収まる）。
SSB では FM-threshold floor が消えるので modem は本来の限界
~−3.7 dB SNR_3kHz Robust 閾値で動作可能 — 特に HF で有用な
weak-signal data envelope。

**AFC は 0.3.2 で実装済み**。SSB 用途には
...
を使う。デフォルトの `decode_known_layout` は引き続き
`audio_centre_hz` を厳密に既知と仮定 (NFM では TX/RX が同じ
...
### 3.6 マルチチャンネル SSB の余地 (0.3.3 候補)

uvpacket 信号 1 つの占有帯域は `R_s · (1+α) = 1200 · 1.5 =
1800 Hz` (RRC ロールオフ込みの全幅、−3 dB 主ローブは ~600 Hz)。
隣接チャネル干渉 < −20 dB の現実的な間隔は ~1.2 kHz: その間隔で
一方の信号スペクトラムが対向の RRC ロールオフのゼロ点に乗る。

2.4 kHz SSB パスバンドには異なる audio 中心の uvpacket フレーム
**2 つ**が同時に収まる (例: 800 Hz と 2000 Hz)。2.7–3 kHz
SSB ではマージン付きで 2 つ。

0.3.2 の AFC `decode_known_layout_with_afc` がそのビルディング
ブロック: 周波数軸の preamble correlation ピーク探索を、設定
された ±search_hz ウィンドウ内の 1 ピークに絞っているだけ。これを
**任意の探索範囲で閾値以上の局所ピークを全部返す**形に拡張する
のが 0.3.3 の自然な成果物 (`decode_multichannel(audio,
band_lo_hz, band_hi_hz, &afc_opts, &fec_opts) -> Vec<DecodedFrame>`):

- RX 側: AFC の周波数グリッド magnitude を SSB パスバンド全体で
  iterate、信頼度閾値以上の local maxima を取り、それぞれを
  `decode_known_layout_with_afc` に検出した中心周波数で dispatch。
- TX 側: LBT (listen-before-talk) プリミティブ — passband を
  scan、エネルギー最低の 1.2 kHz スロットを見つけて TX。通常の
  時間軸 CSMA/CA を**音声周波数軸**に転置したもの。1 SSB
  チャンネル上の private group 多局メッセージング。

CSMA/CD 本来は半二重 SSB ラジオには適用不能 (送信中の信頼ある
受信ができない)、しかし短い uvpacket バースト (Express ~270 ms、
Robust ~440 ms) では局が TX 試行間に re-scan すれば衝突は稀。
