...
/// Preamble for the [`Mode::Robust`] payload mode (`x⁷ + x⁶ + 1`).
pub const PREAMBLE_ROBUST: [bool; PREAMBLE_LEN] = lfsr_seven(TAPS_X7_X6, 0b000_0001);

/// Preamble for the [`Mode::Standard`] payload mode (`x⁷ + x³ + 1`).
pub const PREAMBLE_STANDARD: [bool; PREAMBLE_LEN] = lfsr_seven(TAPS_X7_X3, 0b000_0001);

/// Preamble for the [`Mode::Fast`] payload mode (`x⁷ + x + 1`).
pub const PREAMBLE_FAST: [bool; PREAMBLE_LEN] = lfsr_seven(TAPS_X7_X1, 0b000_0001);

/// Preamble for the [`Mode::Express`] payload mode
/// (`x⁷ + x⁴ + x³ + x² + 1`).
pub const PREAMBLE_EXPRESS: [bool; PREAMBLE_LEN] = lfsr_seven(TAPS_X7_X4_X3_X2, 0b000_0001);

/// All four preamble variants, indexed in [`Mode`] order so a
/// caller can look up `PREAMBLES[mode as usize]`.
