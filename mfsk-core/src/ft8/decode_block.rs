...
/// Fixed-point per-symbol DFT: i16 audio × Q15 basis, i32-accumulator
/// dot products. Single f32 cast at the end so `compute_llr`'s f32
/// input contract is preserved.
///
/// Two phases per call:
/// 1. **Basis precompute** — 8 tones × {cos, sin} = 16 vectors of
///    NSPS=1920 i16 samples each (60 KB heap, scratch reused across
///    all symbols of this candidate). Generated by Q15 rotator —
///    one cos+sin pair per tone, then 1920 complex multiplies to
///    fill the vector.
/// 2. **Per-symbol dot products** — for each symbol in `mask`,
///    16 calls to [`crate::core::dotprod::dot_q15_i32`]
///    (audio · basis). Default is a Rust loop; embedded targets can
///    override via `mfsk_core_dot_q15_i32` (see
///    `mfsk-core::core::dotprod`) to bridge to chip-native asm
///    (e.g. esp-dsp `dsps_dotprod_s16_ae32` on Xtensa LX6).
///
/// **vs the old fused-rotator inner loop**: separating basis
/// generation from dot product pays a small one-shot cost (basis,
/// ~5 ms on Core2) but unlocks asm dot products (1 cycle/sample on
/// LX6 vs ~14 cycles/sample for the fused Rust loop). Net per-cand
/// stage 3 cost on Core2: 258 ms → ~50 ms with Rust dot, ~10 ms
/// with esp-dsp asm.
#[doc(hidden)]
#[cfg(feature = "fixed-point")]
pub fn fill_symbol_spectra<S: AudioSample>(
